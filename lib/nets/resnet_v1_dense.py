...
class resnetv1_dense(Network):
  ...
  def _build_network(self, sess, is_training=True):
    ...
    if cfg.RESNET.FIXED_BLOCKS > 0:
      with slim.arg_scope(resnet_arg_scope(is_training=False)):
        net_conv, end_points_initial = resnet_v1.resnet_v1(net_conv,
    ...
    with slim.arg_scope(resnet_arg_scope(is_training=is_training)):
      fc7, end_points_last = resnet_v1.resnet_v1(pool5,
                                   blocks[-1:],
                                   global_pool=False,
                                   include_root_block=False,
                                   scope=self._resnet_scope)
      ...
      feature_size = concatenated_features_lower.get_shape().dims[1].value
      print (feature_size)
      # concatenated_features = tf.reshape(tf.tile(concatenated_features_lower, [1, 256]), [-1, tf.shape(concatenated_features_lower)[1]])
      concatenated_features = tf.reshape(tf.tile(concatenated_features_lower, [1, 256]), [256, feature_size])
      print (concatenated_features.get_shape())
      concatenated_features = tf.concat(values=[concatenated_features, tf.reduce_mean(end_points_last['resnet_v1_152/block4'], axis=[1, 2])], axis=1)
      print (concatenated_features.get_shape())
    ...
    with tf.variable_scope(self._resnet_scope, self._resnet_scope):
      ...
      print (fc7.get_shape())
      final_feature_vec = tf.concat(values=[concatenated_features, fc7], axis=1)
