...
def test_net(sess, net, imdb, weights_filename, max_per_image=100, thresh=0.05):
  ...
  # testFile = open('../py-faster-rcnn/data/receipts/data/ImageSets/test_main.txt')
  # testFile = open('/netscratch/siddiqui/Bosch/data/faster-rcnn/data/ImageSets/test_main.txt')
  # testFile = open('/netscratch/siddiqui/Bosch/data/faster-rcnn/schulze/train_cam3_cam4.idl')
  # testFile = open('/netscratch/siddiqui/TableDetection/data/ImageSets/train.txt')
  testFile = open('/netscratch/siddiqui/TableDetection/ICDAR_POD/Test/images.txt')
  imageNames = testFile.readlines()
  counter = 0
  reject_classes = []

  fileAlreadyProcessed = False
  if os.path.isfile("/netscratch/siddiqui/TableDetection/output.txt"):
    f = open("/netscratch/siddiqui/TableDetection/output.txt", "r")
  ...
  for im_name in imageNames:
    ...
    # im_path = im_name
    im_path = '/netscratch/siddiqui/TableDetection/ICDAR_POD/Test/Image/' + im_name
    ...
    if SAVE_SINGLE_IMAGE:
      if True: #len(bboxes) > 0:
        ...
        out_im_path = '/netscratch/siddiqui/TableDetection/output-images/' + im_name
        cv2.imwrite(out_im_path, im)
        print ("Writing output image for file (%d): %s" % (fileIndex, im_name))

        f = open("/netscratch/siddiqui/TableDetection/output.txt", "a+")
