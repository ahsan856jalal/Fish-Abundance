...
class fish(imdb):
                ...
                if isinstance(xmlObjectData, list):
                    for xmlObject in xmlObjectData:
                        if INCLUDE_ALL_INSTANCES_AS_FISH:
                            xmlObject['name'] = self._classes[1] # 'fish'
                        if xmlObject['name'].lower() in self._classes:
                            ...
                            if y2 >= img_shape[0]:
                                y2 = img_shape[0] - 1
                            cls = self._class_to_ind[xmlObject['name'].lower()]
    ...
    def _load_fish_annotation_prev(self, index):
        """
        Load image and bounding boxes info from txt files of fish dataset.
        """
        filename = os.path.join(self._data_path, 'Annotations', index + '.xml')
        with open(filename) as fd:
            doc = xmltodict.parse(fd.read())
            ...
            if xmlObjectName == 'object':
                ...
                if isinstance(xmlObjectData, list):
                    for xmlObject in xmlObjectData:
                        if INCLUDE_ALL_INSTANCES_AS_FISH:
                            xmlObject['name'] = self._classes[1] # 'fish'
                        if xmlObject['name'] in self._classes:
                            # Make pixel indexes 0-based
                            x1 = float(xmlObject['bndbox']['xmin'])
                            y1 = float(xmlObject['bndbox']['ymin'])
                            x2 = float(xmlObject['bndbox']['xmax'])
                            y2 = float(xmlObject['bndbox']['ymax'])
                            cls = self._class_to_ind[xmlObject['name']]
                            # print("%d %d %d %d" % (x1, y1, x2, y2))
                            boxes.append([x1, y1, x2, y2])
                            gt_classes.append(cls)
                            oneHotVec = np.zeros([self.num_classes], dtype=np.float32)
    ...
    def _write_receipts_results_file(self, all_boxes):
        for cls_ind, cls in enumerate(self.classes):
            if cls == '__background__':
                continue
            print 'Writing {} results file'.format(cls)
            filename = self._get_receipts_results_file_template().format(cls)
    ...
    def evaluate_detections(self, all_boxes, output_dir):
        self._write_receipts_results_file(all_boxes)
    ...
    def _get_receipts_results_file_template(self):
